...
class Solution:
    """
        - Time Complexity: O(n^2), n = len(preorder)
        - Space Complexity: O(n), n = len(preorder)
        - Note
            - recursive n times + find index requres O(n) => O(n^2) time complexity
            - if skewed tree, recursive n times => O(n) space complexity
            - if balanced tree, recursive logn times => O(logn) space complexity
    """
